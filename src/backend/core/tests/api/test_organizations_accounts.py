...
def account_test_setup():
    ...
    operator = factories.OperatorFactory(config={"external_management_api_key": "test-external-api-key-12345"})
    operator2 = factories.OperatorFactory(config={"external_management_api_key": "test-external-api-key-abcd"})
...
def test_api_organizations_accounts_create(account_test_setup, auth_method):
    ...
    if auth_method == "user":
        ...
        client.credentials(HTTP_AUTHORIZATION=f"Bearer test-external-api-key-12345")    
    ...
    response = client.post(f"/api/v1.0/operators/{operator.id}/organizations/{organization_ok1.id}/accounts/", data={
    ...
    assert_equals_partial(response.json(), {
        "email": "test@example.com",
        "external_id": "1234567890",
        "type": "user",
        "roles": ["admin"],
    ...
    assert models.Account.objects.get(email="test@example.com").organization == organization_ok1
...
def test_api_organizations_accounts_create_not_allowed_wrong_key(account_test_setup):
    ...
    client = APIClient()
    client.credentials(HTTP_AUTHORIZATION=f"Bearer test-external-api-key-67890")
...
@pytest.mark.parametrize("auth_method", ["user", "external_api_key"])
def test_api_organizations_accounts_create_not_allowed_authent(account_test_setup, auth_method):
    ...
    response = client.post(f"/api/v1.0/operators/{operator.id}/organizations/{organization.id}/accounts/", data={
        "email": "test@example.com",
        "external_id": "1234567890",
        "type": "user",
        "roles": ["admin"],
...
def test_api_organizations_accounts_list(account_test_setup, auth_method):
    ...
    response = client.get(f"/api/v1.0/operators/{operator.id}/organizations/{organization_ok1.id}/accounts/")
...
@pytest.mark.parametrize("auth_method", ["user", "external_api_key"])
def test_api_organizations_accounts_list_not_allowed_operator(account_test_setup, auth_method):
    ...
    response = client.get(f"/api/v1.0/operators/{operator.id}/organizations/{organization.id}/accounts/")
    assert response.status_code == 403
    assert response.json() == {
        "detail": "Vous n'avez pas la permission d'effectuer cette action."
    }
...
def test_api_organizations_accounts_get(account_test_setup, auth_method):
    ...
    account = factories.AccountFactory(email="test@example.com", organization=organization, external_id="1234567890", type="user", roles=["admin"])
...
def test_api_organizations_accounts_get_operator_do_not_exists(account_test_setup):
    ...
    client = APIClient()
    client.credentials(HTTP_AUTHORIZATION=f"Bearer test-external-api-key-wrong")

    operator = account_test_setup["operator"]
    organization = account_test_setup["organization_ok1"]
    account = factories.AccountFactory(email="test@example.com", organization=organization)
...
@pytest.mark.parametrize("auth_method", ["user", "external_api_key"])
def test_api_organizations_accounts_get_operator_not_allowed(account_test_setup, auth_method):
    """Operators using external API key should not be able to get accounts of other operators."""
    client = APIClient()
    if auth_method == "user":
        client.force_login(account_test_setup['user2'])
    elif auth_method == "external_api_key":
        client.credentials(HTTP_AUTHORIZATION=f"Bearer test-external-api-key-abcd")
...
def test_api_organizations_accounts_patch_account(account_test_setup, auth_method):
    ...
    operator = account_test_setup["operator"]
    organization = account_test_setup["organization_ok1"]
    account = factories.AccountFactory(email="test@example.com", organization=organization, external_id="1234567890", type="user", roles=[])

    response = client.patch(f"/api/v1.0/accounts/{account.id}/", data={
        "roles": ["admin"],
    }, format="json")
    assert response.status_code == 200
    assert_equals_partial(response.json(), {
...
@pytest.mark.parametrize("auth_method", ["user", "external_api_key"])
def test_api_organizations_accounts_patch_account_operator_not_allowed(account_test_setup, auth_method):
...
def test_api_organizations_accounts_patch_account_operator_do_not_exists(account_test_setup):
    """Authenticated auth method (user or operator) should not be able to patch an account of other operators."""
    client = APIClient()
    client.credentials(HTTP_AUTHORIZATION=f"Bearer test-external-api-key-wrong")
...
def test_api_organizations_accounts_patch_service_link(account_test_setup, auth_method):
    ...
    operator = account_test_setup["operator"]
    organization_ok1 = account_test_setup["organization_ok1"]
    account = factories.AccountFactory(email="test@example.com", organization=organization_ok1)
    ...
    response = client.patch(f"/api/v1.0/accounts/{account["id"]}/services/{service1.id}/",
    data={
        "roles": ["admin"],
    }, format="json")
    assert response.status_code == 200
    ...
    assert_equals_partial(account_data, {
        "email": "test@example.com",
        "external_id": "1234567890",
        "type": "user",
        "roles": ["admin"],
...
@pytest.mark.parametrize("auth_method", ["user", "external_api_key"])
def test_api_organizations_accounts_patch_service_link_operator_not_allowed(account_test_setup, auth_method):
    """Authenticated auth method (user or operator) should be able to patch service links of an account."""
    client = APIClient()
    if auth_method == "user":
        client.force_login(account_test_setup['user2'])
