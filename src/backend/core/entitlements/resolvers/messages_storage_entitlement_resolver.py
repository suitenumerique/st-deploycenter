...
class MessagesStorageEntitlementResolver:
    ...
    def resolve(self, context):
        ...
        This method also return in the object the level at which the entitlement was resolved or not via
        "_reason_level" key.
        ...
        if entitlement_organization := entitlements.get("organization"):
            ...
            # If there is a user entitlement and it this one does not resolve, we can return directly.
            # Or if there is no user entitlement to run further, we can return directly in anyway.
            if not resolves or not entitlement_user:
    ...
    def _build_resolve_level(self, attributes, resolve_level):
        ...
        """
        return {**attributes, f"{self.resolve_level_prefix}_resolve_level": resolve_level}
