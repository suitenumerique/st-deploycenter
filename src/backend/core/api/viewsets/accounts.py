...
class OrganizationAccountsViewSet(viewsets.GenericViewSet, mixins.CreateModelMixin, mixins.ListModelMixin):
    ...
    def get_queryset(self):
        return models.Account.objects.filter(organization=self.kwargs["organization_id"])
...
class AccountViewSet(viewsets.GenericViewSet, mixins.RetrieveModelMixin, mixins.UpdateModelMixin):
    ...
    def service_link_update(self, request, *args, **kwargs):
        ...
        service = models.Service.objects.get(id=kwargs["service_id"])
        service_link = account.service_links.filter(service=service).first()
        if not service_link:
            service_link = account.service_links.create(service=service)
        serializer = serializers.AccountServiceLinkSerializer(service_link, data=request.data, partial=True)
